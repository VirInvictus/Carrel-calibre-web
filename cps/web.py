    ...
    if data == "rated":
        ...
        try:
            entries, pagination = quarry_grid.grid(
                page, None, sort=keys, descending=descending
            )
        except Exception:
            # The library vanished mid-request: degrade to the empty grid
            # the old fill_indexpage path produced, never a 500.
            entries, pagination = (
                [],
                quarry_grid.GridPagination(page, config.config_books_per_page or 60, 0),
            )
...
def render_rated_books(page, book_id, order):
    if current_user.check_visibility(constants.SIDEBAR_BEST_RATED):
        # Phase 7: 5-star books (rating 10 on the 0-10 scale) via cquarry.
...
def render_discover_books(book_id):
    if current_user.check_visibility(constants.SIDEBAR_RANDOM):
        # Phase 7: random pick over cquarry's cached ids.
        import random as _random

        all_ids = quarry_grid.all_ids()
        picks = _random.sample(all_ids, min(config.config_books_per_page, len(all_ids)))
        entries, _pagination = quarry_grid.grid(1, picks)
        pagination = Pagination(
...
def render_author_books(page, author_id, order):
    # Phase 7: id set + paging via cquarry; the ORM's series tie-breaks
    # travel as multi-key sorts.
    author_name = quarry_grid.entity_name("authors", author_id)
...
def render_publisher_books(page, book_id, order):
    keys = ("series",) + quarry_grid.search_sort(order[1])[0]
    keys = tuple(dict.fromkeys(keys))
    keys, descending = keys, quarry_grid.search_sort(order[1])[1]
    if book_id == "-1":
        ids = quarry_grid.ids_missing("publishers")
        publisher = _("None")
    else:
        publisher = quarry_grid.entity_name("publishers", book_id)
        if publisher is None:
            abort(404)
        ids = quarry_grid.ids_for_entity("publishers", book_id)
    entries, pagination = quarry_grid.grid(page, ids, sort=keys, descending=descending)
...
def render_series_books(page, book_id, order):
    keys, descending = quarry_grid.search_sort(order[1])
    if book_id == "-1":
        ids = quarry_grid.ids_missing("series")
        series_name = _("None")
    else:
        series_name = quarry_grid.entity_name("series", book_id)
...
def render_ratings_books(page, book_id, order):
    keys, descending = quarry_grid.search_sort(order[1])
    if book_id == "-1":
        ids = quarry_grid.ids_missing("ratings")
        title = _("Rating: None")
    else:
        value = quarry_grid.entity_name("ratings", book_id)
        if value is None:
            abort(404)
        ids = quarry_grid.ids_for_entity("ratings", book_id)
        title = _("Rating: %(rating)s stars", rating=int(value / 2))
    entries, pagination = quarry_grid.grid(page, ids, sort=keys, descending=descending)
    return render_title_template(
        "index.html",
        random=None,
...
def render_formats_books(page, book_id, order):
    ...
    if book_id == "-1":
        name = _("None")
        ids = quarry_grid.ids_missing("formats")
    else:
        fmt = book_id.upper()
        ids = quarry_grid.ids_with("formats", fmt)
        if not ids:
            abort(404)
        name = fmt
    entries, pagination = quarry_grid.grid(page, ids, sort=keys, descending=descending)
...
def render_category_books(page, book_id, order):
    keys = tuple(
        dict.fromkeys(quarry_grid.search_sort(order[1])[0] + ("series", "series_index"))
    )
    keys, descending = keys, quarry_grid.search_sort(order[1])[1]
    if book_id == "-1":
        ids = quarry_grid.ids_missing("tags")
        tagsname = _("None")
    else:
        tagsname = quarry_grid.entity_name("tags", book_id)
        if tagsname is None:
            abort(404)
        ids = quarry_grid.ids_for_entity("tags", book_id)
    entries, pagination = quarry_grid.grid(page, ids, sort=keys, descending=descending)
    return render_title_template(
        "index.html",
        random=None,
...
def render_language_books(page, name, order):
    ...
    keys, descending = quarry_grid.search_sort(order[1])
    if name == "none":
        ids = quarry_grid.ids_missing("languages")
    else:
        ids = quarry_grid.ids_with("languages", name)
    entries, pagination = quarry_grid.grid(page, ids, sort=keys, descending=descending)
    return render_title_template(
        "index.html",
        random=None,
        ...
        try:
            ...
            if read_column_is_enum(config.config_read_column):
                ...
                if are_read:
                    ...
                    db_filter = (
                        coalesce(db.cc_classes[config.config_read_column].value, "")
                        != "Read"
                    )
            elif are_read:
...
for d in data:
    ...
    web.add_url_rule(
        ...
        view_func=books_list,
        defaults={"page": 1, "data": d},
    )
    web.add_url_rule(
        "/{}/<sort_param>/<book_id>/<int:page>".format(d),
        defaults={"data": d},
        view_func=books_list,
    )
